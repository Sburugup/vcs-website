...
const ExecTeam = () => {
  ...
  return (
    <div className="container mx-auto pt-8">
      <h2 className="text-4xl font-bold text-center mb-12 text-slate-900">Executive Board</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {execMembers.map((member, index) => (
          ...
          >
            <img src={member.image} alt={member.name} className="h-40 w-40 rounded-full mx-auto mb-4 border-4 border-amber-400 ring-2 ring-violet-200 object-cover object-center" />
            <div className="flex items-center justify-center gap-2">
              ...
              {member.linkedin && (
                <a 
                  ...
                  className="text-slate-500 hover:text-violet-700 transition-colors duration-200"
                >
                  <LinkedinIcon size={20} />
                </a>
              )}
            </div>
            <p className="text-xl text-violet-800 font-medium">{member.position}</p>
          </motion.div>
        ))}
      </div>
    </div>
